/**
 * 快速重置所有账号的错误状态
 * 使用方法：npx tsx server/reset-all-errors.ts
 */
  ...
  try {
    ...
    for (const account of needReset) {
      try {
        ...
        console.log(`✅ 已重置: ${account.email} (之前失败${account.consecutiveFailures}次)`)
        successCount++
      } catch (error) {
    ...
    console.log(`\n✨ 重置完成！成功: ${successCount}/${needReset.length}`)
    console.log('💡 现在可以重新尝试刷新Token了')
