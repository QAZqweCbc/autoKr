 ...
 * 使用方法: npx ts-node server/scripts/import-from-app-json.ts <json文件路径>
 */
    ...
    for (const account of data.accounts) {
      ...
      try {
        ...
        if (result.success) {
          ...
          console.log(`   支持自动刷新: ${result.account.credentials.refreshToken ? '是' : '否'}`)
          successCount++
        } else {
...
if (args.length === 0) {
  console.log(`
使用方法:
  npx ts-node server/scripts/import-from-app-json.ts <json文件路径>

示例:
  npx ts-node server/scripts/import-from-app-json.ts kiro-accounts-2026-02-07.json
  npx ts-node server/scripts/import-from-app-json.ts E:\\git\\Kiro-auto-register\\kiro-accounts-2026-02-07.json
