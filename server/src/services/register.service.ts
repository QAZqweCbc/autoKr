...
import { emitTaskUpdate, emitTaskLog, emitAccountUpdate, emitRegistrationSuccess, emitRegistrationFailure } from '../websocket/socket.handler'
...
async function executeTask(task: Task) {
  ...
  try {
    ...
    if (result.success) {
      ...
      // 📡 推送注册失败事件到前端（显示在"注册账户日志"中）
      emitRegistrationFailure({
        taskId: task.id,
        email: task.email,
        password: task.password,
        name: result.name,
