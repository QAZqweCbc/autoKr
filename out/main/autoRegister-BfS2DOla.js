...
async function randomDelay(minSeconds = 3, maxSeconds = 8, log, description, configuredMin, configuredMax) {
  const actualMin = configuredMin !== void 0 ? configuredMin : minSeconds;
  const actualMax = configuredMax !== void 0 ? configuredMax : maxSeconds;
  const delaySeconds = actualMin + Math.random() * (actualMax - actualMin);
  const delayMs = Math.round(delaySeconds * 1e3);
  if (log && description) {
    log(`${description}（随机等待 ${(delayMs / 1e3).toFixed(1)} 秒）...`);
  }
...
async function humanPageBehavior(page, log) {
  ...
  if (Math.random() < 0.3) {
    ...
    await page.waitForTimeout(500 + Math.random() * 1e3);
  }
...
async function injectCanvasNoise(page) {
  await page.addInitScript(() => {
    ...
    function addNoise(imageData) {
      ...
      for (let i = 0; i < data.length; i += 4) {
        if (Math.random() < 0.1) {
          data[i] = data[i] ^ (noiseSeed > 0.5 ? 1 : 0);
          data[i + 1] = data[i + 1] ^ (noiseSeed > 0.3 ? 1 : 0);
          data[i + 2] = data[i + 2] ^ (noiseSeed > 0.7 ? 1 : 0);
        }
      }
      return imageData;
    }
...
function htmlToText(html) {
  if (!html) return "";
  let text = html;
  text = text.replace(/&nbsp;/g, " ").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&apos;/g, "'").replace(/&#(\d+);/g, (_, n) => String.fromCharCode(parseInt(n))).replace(/&#x([0-9a-fA-F]+);/g, (_, n) => String.fromCharCode(parseInt(n, 16)));
...
async function testQQEmailConnection(email, authCode, log) {
  log("========== 测试 QQ 邮箱连接 ==========");
  log(`邮箱: ${email}`);
...
async function getQQVerificationCodeWithRetry(qqEmail, authCode, log, timeout = 120, targetEmail, maxRetries = 2) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const isRateLimitError = errorMsg.includes("Account is abnormal") || errorMsg.includes("login frequency limited") || errorMsg.includes("Login fail");
...
async function getImapVerificationCode(config, log, timeout = 120, targetEmail) {
      ...
      imap.openBox("INBOX", false, (err) => {
        ...
        const checkMail = () => {
          ...
          imap.search(["UNSEEN"], (err2, results) => {
            if (err2) {
              log(`搜索邮件失败: ${err2}，尝试搜索所有邮件...`);
              imap.search(["ALL"], (err22, results2) => {
                if (err22) {
                  log(`搜索所有邮件也失败: ${err22}`);
                  setTimeout(checkMail, 5e3);
                  return;
                }
        ...
        const processResults = (results) => {
          ...
          const fetch2 = imap.fetch(recentResults, { bodies: "" });
          let foundCount = 0;
          fetch2.on("message", (msg) => {
            msg.on("body", (stream) => {
              mailparser.simpleParser(stream, (err2, parsed) => {
                if (err2 || resolved) return;
                ...
                const isAwsMail = AWS_SENDERS.some(
...
async function getQQVerificationCode(qqEmail, authCode, log, timeout = 120, targetEmail) {
...
async function getGmailVerificationCode(gmailAddress, appPassword, log, timeout = 120, targetEmail) {
  return getImapVerificationCode(
    {
      user: gmailAddress,
...
async function getOutlookVerificationCode(refreshToken, clientId, log, timeout = 120) {
  log("========== 开始获取邮箱验证码 ==========");
...
========== 找到验证码: ${code} ==========`);
      ...
      log(`未找到验证码，${checkInterval / 1e3}秒后重试...`);
      await new Promise((r) => setTimeout(r, checkInterval));
    } catch (error) {
      log(`获取验证码出错: ${error}`);
      await new Promise((r) => setTimeout(r, checkInterval));
...
async function waitAndFill(page, selector, value, log, description, timeout = 3e4) {
...
async function tryClickSelectors(page, selectors, log, description, timeout = 15e3) {
...
async function checkAndRetryOnError(page, buttonSelector, log, description, maxRetries = 3, retryDelay = 2e3) {
...
async function waitAndClickWithRetry(page, selector, log, description, timeout = 3e4, maxRetries = 3) {
  log(`等待${description}出现...`);
  try {
    const element = page.locator(selector).first();
    await element.waitFor({ state: "visible", timeout });
...
async function activateOutlook(email, emailPassword, log) {
  ...
  try {
    ...
    await page.goto(activationUrl, { waitUntil: "networkidle", timeout: 6e4 });
    log("✓ 页面加载完成");
    ...
    for (const selector of passwordInputSelectors) {
      try {
        const element = page.locator(selector).first();
        await element.waitFor({ state: "visible", timeout: 15e3 });
    ...
    try {
      ...
      await skipElement.waitFor({ state: "visible", timeout: 3e4 });
    ...
    try {
      const skipElement = page.locator(skipSelector).first();
      await skipElement.waitFor({ state: "visible", timeout: 15e3 });
      await skipElement.click();
      log('✓ 已点击第二个"暂时跳过"');
      await page.waitForTimeout(3e3);
    } catch {
    ...
    if (!await tryClickSelectors(page, cancelButtonSelectors, log, '"取消"按钮', 15e3)) {
    ...
    if (!await tryClickSelectors(page, yesButtonSelectors, log, '"是"按钮', 15e3)) {
    ...
    for (const selector of newMailSelectors) {
      try {
        const element = page.locator(selector).first();
        await element.waitFor({ state: "visible", timeout: 3e4 });
        log("✓ Outlook 邮箱激活成功！");
        outlookLoaded = true;
        break;
      } catch {
...
async function autoRegisterAWS(email, password, refreshTokenOrAuthCode, clientId, log, emailPassword, skipOutlookActivation = false, proxyUrl, receiveEmail, browserConfig) {
  ...
  try {
    ...
    if (isCustomFirefox) {
      ...
      browserModule = playwright.chromium;
    } else {
      switch (browserType) {
        case "firefox":
          const { firefox } = await import("playwright");
          browserModule = firefox;
          break;
        default:
          browserModule = playwright.chromium;
      }
    ...
    const launchOptions = {
      ...
      proxy: proxyUrl ? { server: proxyUrl } : void 0
    };
    ...
    try {
      ...
      page.on("response", (response2) => {
        ...
        if (status >= 300 && status < 400) {
          redirectCount++;
          log(`🔄 检测到重定向 #${redirectCount}: ${status} ${response2.url()} → ${response2.headers()["location"] || "未知"}`);
        }
      });
      const response = await page.goto(registerUrl, { waitUntil: "domcontentloaded", timeout: 6e4 });
      ...
      log("等待页面完全加载...");
      await page.waitForLoadState("networkidle", { timeout: 3e4 });
      await page.waitForTimeout(2e3);
      log("✓ 页面加载完成");
    } catch (error) {
      log(`⚠️ 页面加载异常: ${error}`);
    ...
    try {
      ...
      const result = await Promise.race([
        loginHeading.waitFor({ state: "visible", timeout: 1e4 }).then(() => "login"),
        verifyHeading.waitFor({ state: "visible", timeout: 1e4 }).then(() => "verify"),
        verifyCodeInput.waitFor({ state: "visible", timeout: 1e4 }).then(() => "verify-input"),
        nameInput.waitFor({ state: "visible", timeout: 1e4 }).then(() => "register")
      ]);
      if (result === "login") {
        isLoginFlow = true;
      ...
      try {
        await page.locator(loginHeadingSelector).first().waitFor({ state: "visible", timeout: 3e3 });
        isLoginFlow = true;
      } catch {
        try {
    ...
    if (isLoginFlow) {
      ...
      for (const selector of loginCodeSelectors) {
        try {
          await page.locator(selector).first().waitFor({ state: "visible", timeout: 1e4 });
      ...
      let loginVerificationCode = null;
      if (isCodeQQEmail) {
      ...
      log("\n步骤2: 输入姓名...");
      await page.locator(nameInputSelector).first().waitFor({ state: "visible", timeout: 3e4 });
      ...
      for (const selector of codeInputSelectors) {
        try {
          ...
          if (elements > 0) {
            await page.locator(selector).first().waitFor({ state: "visible", timeout: 5e3 });
      ...
      if (!codeInputSelector) {
        ...
        const allInputs = await page.locator("input").evaluateAll(
      ...
      log("等待密码输入框出现...");
      await page.locator(passwordInputSelector).first().waitFor({ state: "visible", timeout: 6e4 });
      ...
      log("等待确认密码输入框出现...");
      await page.locator(confirmPasswordSelector).first().waitFor({ state: "visible", timeout: 6e4 });
    ...
    for (const selector of confirmButtonSelectors) {
      try {
        ...
        await confirmButton.waitFor({ state: "visible", timeout: 15e3 });
        const pageContent = await page.content();
    ...
    if (!confirmClicked) {
      ...
      for (const selector of allowButtonSelectors) {
        try {
          ...
          await allowButton.waitFor({ state: "visible", timeout: 15e3 });
          const pageContent = await page.content();
          if (pageContent.includes("Allow AWS Builder ID Registration to access your data") || pageContent.includes("Kiro")) {
            log("✓ 检测到 Kiro 授权页面");
          }
    ...
    for (const indicator of successIndicators) {
      try {
        await page.locator(indicator).first().waitFor({ state: "visible", timeout: 5e3 });
        log("✓ 检测到成功页面");
        foundSuccess = true;
        break;
      } catch {
...
exports.activateOutlook = activateOutlook;
exports.autoRegisterAWS = autoRegisterAWS;
exports.getGmailVerificationCode = getGmailVerificationCode;
exports.getOutlookVerificationCode = getOutlookVerificationCode;
exports.getQQVerificationCode = getQQVerificationCode;
exports.testQQEmailConnection = testQQEmailConnection;
