/**
 * autoRegister 代理模块
 * 运行时动态加载 server 内的 autoRegister，兼容源码和编译产物
 */
...
function patchPlaywrightScreenshotPath(autoRegisterPath: string) {
  ...
  // 优先从 autoRegister 所在目录和 server 目录查找 playwright
  const searchDirs = [
    path.dirname(autoRegisterPath),
    path.resolve(__dirname, '../..'),  // server 目录
    path.resolve(__dirname, '../../..') // 兼容旧运行位置
  ]
  ...
  if (!playwrightPath) {
    ...
    console.log('⚠ playwright 未安装，跳过调试截图 patch')
    return
  }
  ...
  console.log(`📸 调试截图目录: ${getDebugImageDir()}`)
}
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (fs.existsSync(packagePath)) {
      try {
        ...
        if (pkg.name === 'kiro-account-server') {
          return currentDir
        }
      } catch {
...
export async function getAutoRegisterAWS() {
  ...
  // 优先加载 server 编译产物，开发环境下回退到 TypeScript 源码
  ...
  if (fs.existsSync(compiledAutoRegisterPath)) {
    ...
    console.log('找到 server 编译后的 autoRegister 模块')
  }

  if (!autoRegisterPath) {
    console.log('未找到编译版本，尝试加载 TypeScript 源码...')
  ...
  try {
    ...
    if (autoRegisterPath.endsWith('.ts') && error.code === 'MODULE_NOT_FOUND') {
      ...
      throw new Error('请编译项目后重试: npm run build')
    }
    ...
    console.error(`项目目录: ${serverDir}`)
    console.error('\n可能的原因：')
    console.error('  1. 项目未编译（dist/main/autoRegister.js 不存在）')
    ...
    console.error('  npm run build         # 编译项目')
    console.error('  npm run start         # 启动项目')
    console.error('='.repeat(60) + '\n')
    throw error
  }
}
